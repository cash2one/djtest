...
class UnproperResponse(HttpResponse):
    temp = Template('<h1><pre style="text-align:center">{{message}}</pre></h1>')
...
class ModelView(View):
    ...
    def get_context_data(self, **kwargs):
        context = {
            'view'  : self,
            'appurl': self.rvs, 
            'appname'   : self.appname or self.model and self.model._meta.app_label or '',
        }
    ...
    def rvs(self, name, **kwargs):
        '针对app的reverse'
        return reverse('%s:%s'%(self.appname or self.model._meta.app_label, name), kwargs=kwargs)
